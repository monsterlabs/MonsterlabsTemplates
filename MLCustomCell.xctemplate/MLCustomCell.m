...
#import "MLCustomCell.h"
#import "MLCustomCellBackground.h"

const CGFloat MLCustomCellDefaultRowHeight = 44.0;

@implementation MLCustomCell
...
//
// MLCustomCellBackgroundClass
//
// returns the subclass of MLCustomCellBackground used for drawing the background
//
+ (Class)CustomCellBackgroundClass
{
  return [MLCustomCellBackground class];
}
...
+ (CGFloat)rowHeight
{
  ...
  if (rowHeight == nil)
  {
    ...
    if (nibName)
    {
      ...
      MLCustomCell *cellInstance = [[[[self class] alloc] init] autorelease];
      [[NSBundle mainBundle]
    ...
    {
      return MLCustomCellDefaultRowHeight;
    }
  }
  ...
  indexPath:(NSIndexPath *)anIndexPath
{
  ...
  {
    MLCustomCellBackground *cellBackgroundView =
      [[[[[self class] CustomCellBackgroundClass] alloc]
        initSelected:NO
        grouped:aTableView.style == UITableViewStyleGrouped]
      autorelease];
    self.backgroundView = cellBackgroundView;
    MLCustomCellBackground *cellSelectionView =
      [[[[[self class] CustomCellBackgroundClass] alloc]
        initSelected:YES
        grouped:aTableView.style == UITableViewStyleGrouped]
      autorelease];
  ...
  {
    MLCustomCellGroupPosition position =
      [MLCustomCellBackground positionForIndexPath:anIndexPath inTableView:aTableView];
    ((MLCustomCellBackground *)self.backgroundView).position = position;
    ((MLCustomCellBackground *)self.selectedBackgroundView).position = position;
  }
}
...
{
    [aTableView
     deselectRowAtIndexPath:[aTableView indexPathForSelectedRow]
     animated:YES];
}
